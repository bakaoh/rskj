...
public class LockingCapTest extends BridgePerformanceTestCase {
    ...
    public void getLockingCap() {
        ...
        executeTestCase((int executionIndex) -> Bridge.GET_LOCKING_CAP.encode(), "getLockingCap", 200, stats);
    ...
    public void increaseLockingCap() {
        ...
        // Casting to int seems awful but the value for initialLockingCap won't be anything above 21M
        int minValue = (int)BridgeRegTestConstants.getInstance().getInitialLockingCap().value;
        executeTestCase(
                (int executionIndex) -> {

                    long value = Helper.randomCoin(Coin.COIN, minValue, minValue * 10).getValue();
